(function () {
  ...
  var ZcuiWcSearchWidget = function (_CustomElement2) {
    inherits(ZcuiWcSearchWidget, _CustomElement2);
    createClass(ZcuiWcSearchWidget, null, [{
    ...
    function ZcuiWcSearchWidget() {
      classCallCheck(this, ZcuiWcSearchWidget);

      var _this = possibleConstructorReturn(this, (ZcuiWcSearchWidget.__proto__ || Object.getPrototypeOf(ZcuiWcSearchWidget)).call(this));

      _this.updateShadowDom = _this.updateShadowDom.bind(_this);
      ...
      _this.searchCar = _this.searchCar.bind(_this);
      _this._validateParams = _this._validateParams.bind(_this);
      _this.filterLocations = _this.filterLocations.bind(_this);
      _this.changeLocation = _this.changeLocation.bind(_this);
      _this.closeLocationList = _this.closeLocationList.bind(_this);
      ...
      _this._loadXMLDoc({
        ...
        if (data) {
          _this.cities = JSON.parse(data).cities.sort(function (a, b) {
        ...
        if (err) _this.apiErrorMsg = JSON.parse(err).msg;
        _this.updateShadowDom();
      });

      _this.locations = {};
      _this.filteredLocation = [];

      var monthNames = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
      var today = new Date();
      _this.monthsYears = Array.apply(null, { length: 6 }).map(function (x, i) {
      ...
      _this.timeList = Array.apply(null, { length: 48 }).map(function (x, i) {
        var temp = i;
        var ampm = 'AM';
        if (i >= 24) {
          temp = i - 24;
      ...
      var tomorrow = new Date(today.getTime() + 60 * 60 * 1000);
      var nextDayTomorrow = new Date(tomorrow.getTime() + 24 * 60 * 60 * 1000);
      //default values
      _this.searchParams = {
        starts: {
          date: tomorrow.getDate(),
          monthYearIndex: tomorrow.getMonth() > today.getMonth() ? 1 : 0,
          time: _this._get12HrTime(tomorrow)
        },
        ends: {
          date: nextDayTomorrow.getDate(),
          monthYearIndex: nextDayTomorrow.getMonth() > tomorrow.getMonth() ? 1 : 0,
          time: _this._get12HrTime(nextDayTomorrow)
        },
        cityLinkName: 'mumbai',
        cityName: 'Mumbai'

      };
      _this._updateLocations();

      // index 0 means no error
      _this.errorMessages = ['', 'Please select city', 'Please select starting point', 'Please select start date', 'Please select start month', 'Please select start time', 'Please select end date', 'Please select end month', 'Please select end time', 'Starts Can\'t be in past', 'ends Can\'t be in past', 'Wrong start date selected', 'Wrong end date selected', 'Start date cannot be greater than end date'];
      _this.pickupErrors = [1, 2];
      _this.startsErrors = [3, 4, 5, 9, 11, 13];
      _this.endsErrors = [6, 7, 8, 10, 12, 13];
      return _this;
    }

    createClass(ZcuiWcSearchWidget, [{
      key: 'createShadowDom',
      ...
      key: '_updateLocations',
      value: function _updateLocations() {
        var _this2 = this;
        ...
        this._loadXMLDoc({
          ...
          if (data) {
            var hubs = JSON.parse(data).hubs.sort(function (a, b) {
              var nameA = a.name.toLowerCase(),
                  nameB = b.name.toLowerCase();
              if (nameA < nameB) //sort string ascending
                return -1;
            ...
            _this2.locations[_this2.searchParams.cityLinkName] = hubs;
          }
          if (err) _this2.apiErrorMsg = JSON.parse(err).msg;
          _this2.updateShadowDom();
        });
      }
    }, {
      key: 'changeCity',
      value: function changeCity(e) {
        ...
        this.searchParams.cityName = this.cities.filter(function (city) {
          return city.link_name == e.target.value;
        })[0].name;
      ...
      key: 'changeDate',
      value: function changeDate(val, type) {
        this.searchParams[type].date = val;
        this.updateShadowDom();
      }
    }, {
      key: 'changeMonth',
      value: function changeMonth(val, type) {
        this.searchParams[type].monthYearIndex = val;
        this.updateShadowDom();
      }
    }, {
      key: 'changeTime',
      value: function changeTime(val, type) {
        this.searchParams[type].time = val;
        this.updateShadowDom();
      }
    }, {
      key: 'filterLocations',
      value: function filterLocations(e) {
        if (!this.searchParams.cityLinkName) return;
        this.filteredLocation = this.locations[this.searchParams.cityLinkName].filter(function (loc) {
      ...
      key: 'changeLocation',
      value: function changeLocation(loc) {
      ...
      key: 'daysInMonth',
      value: function daysInMonth(type) {
        var monthYearIndex = this.searchParams[type].monthYearIndex;
        var selectMonthYear = this.monthsYears[monthYearIndex];
        return new Date(selectMonthYear.year, selectMonthYear.month, 0).getDate();
      }
    }, {
      key: '_get24HrTime',
      value: function _get24HrTime(time) {
        var timeArr = time.split(' ');
        return timeArr[1] == 'PM' ? parseInt(timeArr[0].split(':')[0]) + 12 + ':' + timeArr[0].split(':')[1] : timeArr[0];
      }
    }, {
      key: '_dateInPast',
      value: function _dateInPast(type) {
        var monthYearIndex = this.searchParams[type].monthYearIndex;
        var selectMonthYear = this.monthsYears[monthYearIndex];
        var date = new Date(selectMonthYear.month + '-' + this.searchParams[type].date + '-' + selectMonthYear.year + ' ' + this.searchParams[type].time);
        var today = new Date();
        return today > date;
      }
    }, {
      key: '_isStartsGreaterPast',
      value: function _isStartsGreaterPast() {
        var startsMonthYearIndex = this.searchParams.starts.monthYearIndex;
        var selectStartsMonthYear = this.monthsYears[startsMonthYearIndex];
        var endsMonthYearIndex = this.searchParams.ends.monthYearIndex;
        var selectEndsMonthYear = this.monthsYears[endsMonthYearIndex];
        var starts = new Date(selectStartsMonthYear.month + '-' + this.searchParams.starts.date + '-' + selectStartsMonthYear.year + ' ' + this.searchParams.starts.time);
        var ends = new Date(selectEndsMonthYear.month + '-' + this.searchParams.ends.date + '-' + selectEndsMonthYear.year + ' ' + this.searchParams.ends.time);
        return starts > ends;
      }
    }, {
      key: '_validateParams',
      ...
      key: 'closeLocationList',
      value: function closeLocationList(e) {
      ...
      key: '_get12HrTime',
      value: function _get12HrTime(date) {
      ...
      get: function get$$1() {
        ...
        return html$1(_templateObject, this.closeLocationList, this.errorMessages[this.selectedErrorMessage], this.apiErrorMsg, this.pickupErrors.includes(this.selectedErrorMessage) ? 'input-box error-border' : 'input-box', this.searchParams.cityName ? this.searchParams.cityName : 'Select City', this.changeCity, this.cities.length ? '' : 'Please wait', repeat(this.cities, function (city) {
          return html$1(_templateObject2, city.link_name == _this3.searchParams.cityLinkName ? 'selected' : '', city.link_name, city.name);
        }), this.filterLocations, this.filterLocations, this.searchParams.locationName, this.filteredLocation.length ? 'location-list' : 'hide', repeat(this.filteredLocation, function (loc) {
          return html$1(_templateObject3, function (e) {
            _this3.changeLocation(loc);
          }, loc.name);
        }), this.startsErrors.includes(this.selectedErrorMessage) ? 'input-box error-border' : 'input-box', this.searchParams.starts.date ? this.searchParams.starts.date : 'Date', function (e) {
          _this3.changeDate(e.currentTarget.value, 'starts');
        }, repeat(Array.apply(null, { length: 31 }).map(function (x, i) {
          return i + 1;
        }), function (day) {
          return html$1(_templateObject4, day, day == _this3.searchParams.starts.date ? 'selected' : '', day);
        }), this.searchParams.starts.monthYearIndex || this.searchParams.starts.monthYearIndex == 0 ? this.monthsYears[this.searchParams.starts.monthYearIndex].displayName : 'Month', function (e) {
          _this3.changeMonth(e.currentTarget.value, 'starts');
        }, repeat(this.monthsYears, function (month, i) {
          return html$1(_templateObject5, i == _this3.searchParams.starts.monthYearIndex ? 'selected' : '', i, month.displayName);
        }), this.searchParams.starts.time ? this.searchParams.starts.time : 'Time', function (e) {
          _this3.changeTime(e.currentTarget.value, 'starts');
        }, repeat(this.timeList, function (time) {
          return html$1(_templateObject5, time == _this3.searchParams.starts.time ? 'selected' : '', time, time);
        }), this.endsErrors.includes(this.selectedErrorMessage) ? 'input-box error-border' : 'input-box', this.searchParams.ends.date ? this.searchParams.ends.date : 'Date', function (e) {
          _this3.changeDate(e.currentTarget.value, 'ends');
        }, repeat(Array.apply(null, { length: 31 }).map(function (x, i) {
          return i + 1;
        }), function (day) {
          return html$1(_templateObject6, day == _this3.searchParams.ends.date ? 'selected' : '', day, day);
        }), this.searchParams.ends.monthYearIndex || this.searchParams.ends.monthYearIndex == 0 ? this.monthsYears[this.searchParams.ends.monthYearIndex].displayName : 'Month', function (e) {
          _this3.changeMonth(e.currentTarget.value, 'ends');
        }, repeat(this.monthsYears, function (month, i) {
          return html$1(_templateObject5, i == _this3.searchParams.ends.monthYearIndex ? 'selected' : '', i, month.displayName);
        }), this.searchParams.ends.time ? this.searchParams.ends.time : 'Time', function (e) {
          _this3.changeTime(e.currentTarget.value, 'ends');
        }, repeat(this.timeList, function (time) {
          return html$1(_templateObject5, time == _this3.searchParams.ends.time ? 'selected' : '', time, time);
        }), this.searchCar);
      }
    }]);
    return ZcuiWcSearchWidget;
  }(_CustomElement);
